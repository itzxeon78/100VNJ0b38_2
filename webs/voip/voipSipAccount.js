...
function codecChange()
{
	...
	if(codecValue1=='None')
	{
		alert("Primary Compression Type must exist");
		codec1.value = recoverCodec1;
	}else if(codecValue1==codecValue2)
	{
...
function emptyCheck()
{
	...
	checkResult = emptyAndRuleCheck("sipAccountNumber");
	if(checkResult==1)
	{
...
function callForwardCheck()
{
	...
	if(conditional1.checked)
	{
		...
		{
			addErrorMsg("If you enable [Active Unconditional Forward] [To Number] can't be empty\n");
			setFirstErrorEmptyItem("unconditionalNumber","advance");
			isError = true;
		}
	}
		if(conditional2.checked)
		{
			...
			{
				addErrorMsg("If you enable [Active Busy Forward] [To Number] can't be empty\n");
				setFirstErrorEmptyItem("busyNumber","advance");
				isError = true;
			}
		}
		if(conditional3.checked)
		{
			...
			{
				addErrorMsg("If you enable [Active No Answer Forward to Number] [To Number] can't be empty\n");
				setFirstErrorEmptyItem("noAnswerNumber","advance");
				isError = true;
			}
			if(fwdNoAnswerCount != "")
			{
				...
				{
					addErrorMsg("[No Answer Ring Count] is invalid\n");
					setFirstErrorEmptyItem("noAnswerCount","advance");
					isError = true;
				}
			}else
			{
				addErrorMsg("If you enable [Active Unconditional Forward] the [No Answer Ring Count] can't be empty\n");
				setFirstErrorEmptyItem("noAnswerCount","advance");
				isError = true;
			}
		}
...
function formCheck()
{
	...
	/*
	var missedCallEmailTitle = document.getElementById("missedCallEmailTitle").value;
	...
	*/
	...
	if(rangeCheck(1,65535,30,document.getElementById("noAnswerCount"),1,0) == -1){
